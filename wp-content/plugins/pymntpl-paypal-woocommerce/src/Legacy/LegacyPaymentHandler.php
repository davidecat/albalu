...
class LegacyPaymentHandler {
	...
	private function validate_paypal_order( $paypal_order, $order, $order_id_from_cache = false ) {
		...
		// COMPLETED one, so this can't be gated to isComplete() only. custom_id is absent for
		// orders created via the REST-driven cart/express-checkout flow
		// (PurchaseUnitFactory::from_cart() doesn't set it, since no WC order exists yet at that
		// point); those orders are bound to the buyer by the session-cache check further down
		// instead (issue #15 / WPScan 47254).
		$has_custom_id = false;
		$ids_match     = false;
