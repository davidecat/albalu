jQuery(document).ready(function($) {
  ...
  var smartCOD = {
    init: function() {
      ...
      body.on(
        "change",
        ".wc-smart-cod-percantage-switcher",
        smartCOD.switchFee
      );
    ...
    enablePercentage: function(element) {
      var fieldset = element.parents("fieldset"),
        select = $(
          '<select class="wc-smart-cod-percantage-switcher"><option value="fixed">Fixed</option><option value="percentage">%</option></select>'
        ),
        value = "0",
        name = element.attr("name").slice(16);

        var isInputDisabled = fieldset.find("input").is(":disabled");
        ...
        if (splitLabel[0]) {
          ...
          switch (needle) {
            ...
            case "disable": {
              ...
              label.contents().get(0).nodeValue = label
                .text()
                .replace(originalValue, "");
              splitLabel.shift();
              label.prepend(
                '<a class="wc-smart-cod-mode" href="#" data-mode="' +
                  needle +
                  '" data-input-name="' +
                  inputName +
                  '">' +
    ...
    initSelect2: function(element) {
      var action =
          element.data("action") || "woocommerce_json_search_categories",
        security;
      ...
      select2_args = $.extend(
        select2_args,
        smartCOD.getEnhancedSelectFormatString()
      );
    ...
    validateFields: function(extraFields) {
      ...
      extraFields.each(function(index, element) {
        if (
          element.id !== "woocommerce_cod_include_restrict_postals" &&
      ...
      return {
        language: {
          ...
          inputTooLong: function(args) {
            ...
            return smart_cod_variables.enhanced_select.i18n_input_too_long_n.replace(
              "%qty%",
              overChars
            );
          },
          inputTooShort: function(args) {
            ...
            return smart_cod_variables.enhanced_select.i18n_input_too_short_n.replace(
              "%qty%",
              remainingChars
            );
          },
          ...
          maximumSelected: function(args) {
            if (args.maximum === 1) {
              return smart_cod_variables.enhanced_select
                .i18n_selection_too_long_1;
            }

            return smart_cod_variables.enhanced_select.i18n_selection_too_long_n.replace(
              "%qty%",
              args.maximum
            );
          },
