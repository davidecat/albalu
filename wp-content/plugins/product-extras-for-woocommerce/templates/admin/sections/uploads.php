...
<div class="pewc-fields-wrapper pewc-upload-fields split-thirds">
	...
	<div class="product-extra-field pewc-ajax-upload-only">
		<div class="product-extra-field-inner">
			<?php $min_files = ! empty( $item['min_files'] ) ? $item['min_files'] : ''; ?>
			<label>
				<?php _e( 'Min Files', 'pewc' ); ?>
				<?php echo wc_help_tip( 'Minimum number of files if multiple files uploads are enabled', 'pewc' ); ?>
			</label>
		</div>
		<div class="product-extra-field-inner">
			<input type="number" class="pewc-field-item pewc-field-min-files" name="<?php echo esc_attr( $base_name ); ?>[min_files]" min="1" value="<?php echo esc_attr( $min_files ); ?>" data-field-name="min_files">
		</div>
	</div>
