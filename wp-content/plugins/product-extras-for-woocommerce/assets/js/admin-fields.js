...
jQuery( function( $ ) {
  ...
  function set_options_data( option_wrapper ) {
	...
	function pewc_get_value_field_type( field_type ) {
		...
		} else if( field_type == 'select' || field_type == 'calendar-list' || field_type == 'select-box' || field_type == 'radio' || field_type == 'image_swatch' || field_type == 'products' || field_type == 'checkbox_group' || field_type == 'status' || field_type == 'user-role' ) {
	...
	function pewc_add_value_field( field, field_id, field_type, value_field, val ) {
		...
		if( field_type == 'select' || field_type == 'select-box' || field_type == 'radio' || field_type == 'image_swatch' || field_type == 'products' || field_type == 'product-categories' || field_type == 'checkbox_group' ) {
			...
			var options = pewc_populate_calendar_list_value_field( $(field).val() );
			for(var i=0; i < options.length; i++ ) {
				$(clone_value).append($('<option>', {
					value: options[i],
					text: options[i]
  ...
  // Check if our field type allows multiple selections
	...
	// Populate a dynamically added calendar list field
	function pewc_populate_calendar_list_value_field( condition_field ) {
		var data = $('body').find( '#' + condition_field + ' .pewc-data-cl-options' ).attr( 'data-options' );
		if( data == undefined ) {
			data = '[]';
		}
		data = JSON.parse( data );
