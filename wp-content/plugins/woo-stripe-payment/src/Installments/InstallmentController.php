...
class InstallmentController
{
    ...
    private function is_active()
    {
        ...
        return wc_string_to_bool($this->advanced_settings->get_option('installments')) && \in_array($country, ['MX', 'BR']);
    }
    ...
    private function cart_filters_factory()
    {
        ...
        return [new CurrencyFilter($currency, $this->account_settings->get_account_country(wc_stripe_mode())), new OrderTotalFilter(WC()->cart ? WC()->cart->get_total('float') : 0, $currency), new SubscriptionFilter(WC()->cart, null), new PreOrdersFilter(WC()->cart, null)];
    }
    private function order_filters_factory(\WC_Order $order)
    {
        $currency = $order->get_currency();
        return [new CurrencyFilter($currency, $this->account_settings->get_account_country(wc_stripe_order_mode($order))), new OrderTotalFilter($order->get_total(), $currency), new SubscriptionFilter(null, $order), new PreOrdersFilter(null, $order)];
    }
    /**
    ...
    public function add_order_item_total($rows, $order)
    {
        ...
        if ($plan) {
            ...
            $rows[\WC_Stripe_Constants::INSTALLMENT_PLAN] = ['label' => __('Installments:', 'woo-stripe-payment'), 'value' => $this->formatter->format_plan($this->formatter->parse_plan_from_id($plan, true), $amount, $order->get_currency())];
        }
        return $rows;
    }
    /**
    ...
    public function maybe_update_installment_intent()
    {
        ...
        if ($this->intent_needs_update($existing, $params)) {
            $intent = $this->client->paymentIntents->update($existing->id, ['amount' => $params['amount'], 'currency' => $params['currency']]);
    ...
    public function get_or_create_installment_intent($order = null)
    {
        ...
        if ($existing) {
            ...
            if (!is_wp_error($intent) && !in_array($intent->status, ['succeeded', 'requires_capture', 'canceled'], true)) {
                if ($this->intent_needs_update($intent, $params)) {
                    $intent = $this->client->paymentIntents->update($intent->id, ['amount' => $params['amount'], 'currency' => $params['currency']]);
                    if (!is_wp_error($intent)) {
    ...
    {
        $params = ['payment_method_types' => ['card'], 'payment_method_options' => ['card' => ['installments' => ['enabled' => true]]]];
        /**
