...
class PaymentGatewaysController
{
    ...
    public function enqueue_scripts()
    {
        ...
        if ($this->context_handler->has_context(['checkout', 'add_payment_method', 'order_pay']) || apply_filters('wc_stripe_checkout_scripts', false)) {
            ...
            if ($this->context_handler->is_checkout_block() && !$this->context_handler->is_checkout_shortcode() && !$this->context_handler->is_order_pay()) {
                return;
            }
    ...
    public function add_payment_gateway_data(AssetDataApi $asset_data, ContextHandler $context)
    {
        ...
        foreach ($this->registry->get_active_integrations() as $integration) {
             ...
             */
            $data[$integration->id . '_data'] = array_merge(['paymentSections' => $integration->get_option('payment_sections', []), 'paymentMethodType' => $integration->get_payment_method_type(), 'description' => $integration->get_description(), 'hasPaymentTokens' => !is_add_payment_method_page() && !empty($integration->get_tokens()), 'termsDisplayRule' => $terms_rule], $integration->get_payment_method_data());
            $payment_methods[$integration->id] = ['enabled' => wc_string_to_bool($integration->enabled), 'supports' => $integration->supports, 'isLocal' => $integration instanceof \WC_Payment_Gateway_Stripe_Local_Payment, 'paymentMethodType' => $integration->get_payment_method_type()];
        }
        /**
         * @var array                  $data
    ...
    public function add_cart_data($data)
    {
        ...
        $data['paymentMethods'] = [];
        foreach ($this->registry->get_registered_integrations() as $integration) {
            $data['paymentMethods'][$integration->id] = ['id' => $integration->id, 'enabled' => \wc_string_to_bool($integration->enabled), 'available' => isset($available_gateways[$integration->id])];
            /**
