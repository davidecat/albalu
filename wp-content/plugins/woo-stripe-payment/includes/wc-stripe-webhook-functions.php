...
function wc_stripe_process_payment_intent_succeeded($intent, $request, $event)
{
    ...
    if ($payment_method instanceof WC_Payment_Gateway_Stripe) {
        ...
        if ($payment_method->get_payment_method_type() === 'card') {
            WC()->queue()->schedule_single(time() + 2 * MINUTE_IN_SECONDS, 'wc_stripe_process_deferred_webhook', array('type' => $event->type, 'order_id' => $order->get_id(), 'payment_intent' => $intent->id));
        } else {
...
function wc_stripe_process_create_refund($charge)
{
    ...
    try {
        ...
        if (!isset($refund->metadata['order_id'], $refund->metadata['created_via'])) {
            $args = array('amount' => wc_stripe_remove_number_precision($refund->amount, $order->get_currency()), 'order_id' => $order->get_id(), 'reason' => $refund->reason, 'refund_payment' => false);
            ...
            if (!is_wp_error($result)) {
                ...
                $client->refunds->update($refund->id, array('metadata' => array('order_id' => $order->get_id(), 'created_via' => 'stripe_dashboard')));
...
function wc_stripe_charge_dispute_created($dispute)
{
    if (stripe_wc()->advanced_settings->is_dispute_created_enabled()) {
        ...
        if (!$order) {
            ...
            $client->mode($order)->disputes->update($dispute->id, array('metadata' => array('order_id' => $order->get_id(), 'prev_order_status' => $current_status)));
