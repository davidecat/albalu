...
class WC_Stripe_Gateway
{
    ...
    {
        return $this->paymentMethods->all(array('customer' => $customer_id, 'type' => $type), $this->get_api_options($mode));
    }
    ...
    {
        return $this->webhookEndpoints->create(array('url' => $url, 'enabled_events' => $events), $this->get_api_options($mode));
    }
