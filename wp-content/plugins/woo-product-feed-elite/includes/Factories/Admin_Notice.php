...
class Admin_Notice extends Abstract_Class {
    ...
    private function _failed_dependencies() {
        ...
        if ( ! empty( $this->args['failed_dependencies']['missing_plugins'] ) ) {
            foreach ( $this->args['failed_dependencies']['missing_plugins'] as $failed_dependency ) {
        ...
        } elseif ( ! empty( $this->args['failed_dependencies']['failed_version_requirements'] ) ) {
            foreach ( $this->args['failed_dependencies']['failed_version_requirements'] as $failed_dependency ) {
                $failed_dependencies[] = sprintf(
                    /* translators: %1$s = opening <a> tag; %2$s = closing </a> tag */
                    esc_html__(
                        '%1$s%3$s version needs to be on at least version %4$s to work properly with Product Feed Elite. %2$s%1$sPlease update by clicking below.%2$s%5$sUpdate plugin &rarr;%6$s',
                        'woo-product-feed-elite'
                    ),
                    '<p>',
                    '</p>',
    ...
    public function add_notice() {
        ...
        if ( empty( $notice_id ) ) {
            $notice_id = 'adt-pfe-' . md5( is_array( $this->message ) ? wp_json_encode( $this->message ) : $this->message );
        }
