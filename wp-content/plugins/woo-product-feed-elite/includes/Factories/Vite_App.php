...
class Vite_App {
    ...
    public function __construct(
        ...
        $this->base_url = $this->is_hmr
            ? "$protocol//$host:$port/"
            : plugins_url( 'dist/', ADT_PFE_PLUGIN_FILE );
        ...
        if ( ! empty( $l10n ) ) {
            if ( is_admin() ) {
                $this->dependencies = array_merge( $this->dependencies, array( 'woo-product-feed-elite-admin-l10n' ) );
            } else {
                $this->dependencies = array_merge( $this->dependencies, array( 'woo-product-feed-elite-frontend-l10n' ) );
            }
        }
    ...
    protected function parse_env() {
         ...
         */
        if ( ! file_exists( ADT_PFE_PLUGIN_DIR_PATH . '.env' ) ) {
            return;
        }

        //phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
        $env = file_get_contents( ADT_PFE_PLUGIN_DIR_PATH . '.env' );
        if ( $env ) {
            $env = preg_split( '/\\r\\n|\\r|\\n/', $env );
    ...
    public function global_l10n() {
        ...
        $base_handle = 'woo-product-feed-elite';
        if ( is_admin() ) {
            ...
            if ( ! wp_script_is( "$base_handle-admin-l10n" ) ) {
                ...
                wp_localize_script( "$base_handle-admin-l10n", $this->l10n_name, array( 'pluginDirUrl' => plugins_url( '/', ADT_PFE_PLUGIN_FILE ) ) );
            }
        } else {
            ...
            if ( ! wp_script_is( "$base_handle-frontend-l10n" ) ) {
                ...
                wp_localize_script( "$base_handle-frontend-l10n", $this->l10n_name, array( 'pluginDirUrl' => plugins_url( '/', ADT_PFE_PLUGIN_FILE ) ) );
            }
        }
    }
    ...
    public function inject_hmr_config() {
        ...
        $hmr_txt_path = ADT_PFE_PLUGIN_DIR_PATH . 'src/hmr.txt';
    ...
    protected function parse_manifest_file() {
         ...
         */
        $manifest_path = ADT_PFE_PLUGIN_DIR_PATH . 'dist/.vite/manifest.json';
            ...
            foreach ( $imports as $import ) {
                ...
                if ( ! empty( $import_styles ) ) {
                    foreach ( $import_styles as $import_style ) {
                        ...
                        wp_enqueue_style( "$this->script_handle-import-$sanitized_key", $css_url, array(), filemtime( ADT_PFE_PLUGIN_FILE ) );
                    }
                }
            }
        }
    ...
    protected function enqueue_main_script() {
         ...
         */
        if ( null !== $this->l10n ) {
            $handle    = is_admin() ? 'woo-product-feed-elite-admin-l10n' : 'woo-product-feed-elite-frontend-l10n';
        ...
        if ( in_array( 'wp-i18n', $this->dependencies, true ) ) {
            wp_set_script_translations(
                $this->script_handle,
                'woo-product-feed-elite',
                ADT_PFE_PLUGIN_DIR_PATH . 'languages'
            );
        }
        ...
        if ( ! $this->is_hmr ) {
            ...
            if ( ! empty( $styles ) ) {
                foreach ( $styles as $style ) {
                    ...
                    wp_enqueue_style(
                        $style_handle,
                        plugins_url( "dist/$style", ADT_PFE_PLUGIN_FILE ),
                        $this->style_dependencies,
                        filemtime( ADT_PFE_PLUGIN_DIR_PATH . 'dist/' . str_replace( '\\', '/', $style ) )
                    );
                }
        ...
        if ( $context instanceof \WP_Block_Editor_Context && 'my-post-type' === $context->post->post_type ) {
            $categories = array_merge(
                array(
                    array(
                        'slug'  => 'sample-blocks',
                        'title' => __( 'Sample Blocks', 'woo-product-feed-elite' ),
                    ),
                ),
                $categories
            );
    ...
    public function add_script_tag_attributes( $tag, $handle ) {
        ...
        if ( in_array( $handle, $handles, true ) ) {
            if ( $this->is_hmr ) {
                ...
                if ( apply_filters( 'adt_pfe_enable_subresource_integrity_check', defined( 'ADT_PFE_ENABLE_SUBRESOURCE_INTEGRITY_CHECK' ) && ADT_PFE_ENABLE_SUBRESOURCE_INTEGRITY_CHECK ) ) {
                    $integrity = sprintf(
