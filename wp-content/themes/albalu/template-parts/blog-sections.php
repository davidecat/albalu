<?php
/**
 * Blog sections: Newsletter, Features, Categories, Featured Products
 * Features & Categories: always from Chi Siamo (home page data)
 * Newsletter & Featured Products: from Posts page (if added)
 */
...
// Part 1: Loop through Posts page sections (newsletter, most_requested_products only)
if ( function_exists( 'have_rows' ) && have_rows( 'chi_siamo_sections', $section_post_id ) ) :
...
<?php
// Part 2: Always render Features from Chi Siamo (home page data)
$features_section = function_exists( 'albalu_get_default_features_section' ) ? albalu_get_default_features_section() : array( 'items' => array(), 'bg_color' => '#f8f9fa' );
$features_items    = $features_section['items'] ?? array();
$features_bg      = $features_section['bg_color'] ?? '#f8f9fa';
if ( ! empty( $features_items ) ) : ?>
<section class="features-section py-5" style="background-color:<?php echo esc_attr( $features_bg ); ?>;">
    <div class="container">
        <div class="row g-4">
...
<?php
// Part 3: Always render Categories from Chi Siamo (home page data)
$cat_defaults = function_exists( 'albalu_get_default_categories_grid' ) ? albalu_get_default_categories_grid() : array();
$cat_title    = $cat_defaults['title'] ?? '';
$cat_subtitle = $cat_defaults['subtitle'] ?? '';
$cat_items    = $cat_defaults['items'] ?? array();
$cat_bg       = $cat_defaults['bg_color'] ?? '#ffffff';
if ( ! empty( $cat_items ) ) : ?>
<section class="chi-categories py-5" style="background-color:<?php echo esc_attr( $cat_bg ); ?>;">
    <div class="container">
        <?php if ( $cat_title ) : ?>
            <h2 class="text-uppercase fw-bold fs-5 letter-spacing-2 mb-4 text-center"><?php echo wp_kses_post( $cat_title ); ?></h2>
        <?php endif; ?>
        <?php if ( $cat_subtitle ) : ?>
            <h2 class="fw-normal mb-2 border-bottom text-center"><?php echo wp_kses_post( $cat_subtitle ); ?></h2>
        <?php endif; ?>
